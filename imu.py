...
class IMU:
    ...
    def get_orientation(self, angle=False):
        if not angle:
            ...
            angles = R.from_quat(np.hstack([self.gt_data[:, 5].reshape((-1, 1)), self.gt_data[:, 6].reshape((-1, 1)), self.gt_data[:, 7].reshape((-1, 1)), self.gt_data[:, 4].reshape((-1, 1))])).as_euler(seq='xyz')
